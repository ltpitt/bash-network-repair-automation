...
# We put into a variable its content
...
# If network test failed more than 5 times
...
# We restart wlan0
    ...
    sleep 30
# Then we check again if restarting wlan0 fixed the issue, if not we reboot and forcefsck
    host_status=$(fping $gateway_ip)
    if [[ $host_status == *"alive"* ]]
    then
        echo "Network is working correctly" && echo 0 > $network_check_tries_file
